...
const INVALID_UPDATER_ERROR =
  "Can't send request to updater because the native updater isn't loaded.";

function InitUpdater(response_handler, ...args) {
    Logger.Updater("Updater Started.");
...
class Updater extends EventEmitter {
  constructor(options = {}) {
    ...
    if (nativeUpdaterModule == null) {
      try {
        nativeUpdaterModule = require("./updater.js");
      } catch (err) {
    ...
    this.nativeUpdater = InitUpdater({
  ...
  _sendRequestSync(detail) {
    ...
    return this.nativeUpdater.command_blocking(
      JSON.stringify([requestId, detail])
    );
  }
    ...
    try {
      ...
      if (request == null) {
        Logger.Updater(
          ...
          " for a request (",
          id,
          ") not in the updater request map."
        );
        return;
      }
      ...
      if (detail["Error"] != null) {
        ...
        Logger.Log("Unknown updater response", detail);
      }
    } catch (e) {
      Logger.Error("Unhandled exception in updater response handler:", e);
  ...
  _startCurrentVersionInner(options, versions) {
    ...
    if (
      path.resolve(hostExePath) != path.resolve(process.execPath) &&
      ...
      Logger.Log(
        `Restarting from ${path.resolve(process.execPath)} to ${path.resolve(
          hostExePath
        )}`
      );
      app.quit();
  ...
  _commitModulesInner(versions) {
    ...
    for (const module in versions.current_modules) {
      ...
      const moduleSearchPath = path.join(
        modulesPath,
        `${module}-${moduleVersion}`
      );

      if (
        !this.committedModules.has(module) &&
        NodeModule.globalPaths.indexOf(moduleSearchPath) === -1
      ) {
  ...
  queryCurrentVersionsSync() {
    return this._handleSyncResponse(
      this._sendRequestSync("QueryCurrentVersions")
    );
  }
